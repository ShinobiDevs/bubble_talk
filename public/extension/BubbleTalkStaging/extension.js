appAPI.ready(function($) {
	...
	'<div id="social-sign-up" class="bubbleTalk-border">' +
		...
		//'<label>Please <a href="http://bubbletalk.herokuapp.com">sign in</a></label>' +
	...
	'<div id="bubbleTalk-actions" class="bubbleTalk-border">' +
		...
		//'<a href="https://www.facebook.com/shrer/sharer.php?u=http://bubbletalk.herokuapp.com" class="btn btn-primary" target="_blank" id="facebook_share">' +
		'<a href="https://www.facebook.com/shrer/sharer.php?u=http://localhost:3000" class="btn btn-primary" target="_blank" id="facebook_share">' +
	...
	function load_share() {
		$.ajax({
			type: "GET",
			//url: "http://bubbletalk.herokuapp.com/v1/shares/" + current_user.id.$oid + ".json",
	...
	function load_clicked_share(share_uuid) {
		$.ajax({
			type: "GET",
			//url: "http://bubbletalk.herokuapp.com/shares/" + current_user.id.$oid + ".json",
	...
	function load_user(user_token) {
		$.ajax({
			type: "GET",
			//url: "http://bubbletalk.herokuapp.com/users/me.json?user_token=" + user_token,
			...
			error: function (data) {
				appAPI.db.async.remove('user_access_token');
	...
	function handleNewShare(shareJSON) {
		$.ajax({
			type: "POST",
			//url: "http://bubbletalk.herokuapp.com/v1/shares.json",
	...
	function handleDeleteShare() {
		...
		$.ajax({
			type: "DELETE",
			//url: "http://bubbletalk.herokuapp.com/v1/shares/" + share_uuid + ".json",
	...
	$("#facebook_share").click(function(e){
		e.preventDefault();
	    //window.open('https://www.facebook.com/sharer/sharer.php?u=http://bubbletalk.herokuapp.com/shares/'+Share.getInstance().uuid, "facebook-share-dialog", 'width=626,height=436');
	    window.open('https://www.facebook.com/sharer/sharer.php?u=http://localhost:3000/shares/'+Share.getInstance().uuid, "facebook-share-dialog", 'width=626,height=436');
  	})
});